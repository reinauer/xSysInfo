...
| --- | --- |
| `DISPLAY=auto` | Default. Open a window if the detected Workbench screen is wider than 640 pixels or taller than 512 pixels; otherwise use a separate screen with system defaults. |
...
`AUTO` and `SCREEN` respect AmigaOS Mode Promotion. `PAL` and `NTSC`
explicitly select native timing on AmigaOS 2.0 or later. On Kickstart 1.3,
they behave like `SCREEN`, using the system's native PAL or NTSC mode.
